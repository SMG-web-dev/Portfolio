    ...
    <motion.div
      ...
      className={`relative rounded-3xl p-6 sm:p-7 backdrop-blur-md transition-all duration-300 ${
        isAICategory
          ...
          : "bg-white/80 border border-white/50 shadow-card hover:shadow-card-hover text-brunswick-green dark:bg-[#111e16] dark:border-white/10 dark:text-white"
      }`}
    >
      {/* Category Header */}
      <div className="flex items-center justify-between mb-6">
        <h3
          className={`text-xl sm:text-2xl font-display font-bold ${
            isAICategory ? "text-white" : "text-brunswick-green dark:text-white"
          }`}
        >
          {t(category.name)}
        </h3>
